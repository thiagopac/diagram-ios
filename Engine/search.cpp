...
finalize:
  ...
  // PUSH iPHONE
  bestmove_to_ui(move_to_uci(RootMoves[0].pv[0], RootPos.is_chess960()),
                 move_to_uci(RootMoves[0].pv[1], RootPos.is_chess960()));
  // POP iPHONE
}
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // PUSH iPHONE
    if (pos.this_thread() == Threads.main()) Thread0Nodes++;
    // POP iPHONE
...
moves_loop: // When in check and at SpNode search starts from here
    ...
    while ((move = mp.next_move<SpNode>()) != MOVE_NONE)
    {
      ...
      if (RootNode)
      {
          ...
          if (thisThread == Threads.main() && Time::now() - SearchTime > 3000)
              ...
              // PUSH iPHONE
              currmove_to_ui(move_to_uci(move, false), (int)moveCount + (int)PVIdx,
                   (int)RootMoves.size(), int(depth / ONE_PLY));
              // POP iPHONE          
      }
  ...
  string uci_pv(const Position& pos, int depth, Value alpha, Value beta) {
    ...
    for (size_t i = 0; i < uciPVSize; ++i)
    {
        ...
        // PUSH iPHONE
        pv_to_ui(pv_to_ios(&RootMoves[i].pv[0]), d, score_to_ios(v), scoretype_to_ios(v, alpha, beta),
...
void check_time() {
  ...
  if (Time::now() - lastInfoTime >= 1000)
  {
      ...
      dbg_print();
      // PUSH iPHONE
      searchstats_to_ui(Threads.size() * Thread0Nodes, Time::now() - SearchTime);
      // POP iPHONE      
  }
